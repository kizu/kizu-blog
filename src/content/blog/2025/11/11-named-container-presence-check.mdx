...
description: 'I was doing some styling recently, and I wanted to know if my element is inside a certain container. While I could reuse the same selector that I used to apply the container with, this did not feel right: what if I would apply the same container via a different selector? Or have a version of that element where the container is removed via some modifier? Can we somehow use just the container’s name for this?'
...
I was doing some _styling_ recently, and I wanted to know if my element is inside a certain container.
...
While I could reuse the same selector that I used to apply the container with, this did not feel right: what if I would apply the same container via a different selector? Or have a version of that element where the container is removed via some modifier?
...
I will open a CSSWG issue about this. If you have any use cases for this, write about them in this issue, otherwise, if you think it is a good idea — please, upvote it!
