...
One thing you could notice: I have an extra `.u` span inside the label and summary, wrapping the actual text content.
...
In all browsers, the chheckbox is treated as an inline element, and Firefox also treats the `summary`’s marker as such, and thus the `text-decoration` might go over them.
