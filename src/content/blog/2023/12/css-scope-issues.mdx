...
This article did clarify a few things for me, but also made me think of one potential usage of the `:scope` pseudo-class: “storing” the scoping root’s selector as a reusable entity, covering the use case which is available in some of the CSS preprocessors: [`@at-root` in Sass](https://sass-lang.com/documentation/at-rules/at-root/) and [root reference in Stylus](https://stylus-lang.com/docs/selectors.html#root-reference). We don’t have anything like this with native CSS nesting, so the possibility to use scopes in this way seemed like a really neat feature.
