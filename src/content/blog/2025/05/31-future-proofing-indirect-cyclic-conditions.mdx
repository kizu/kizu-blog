...
## Update from 2026-08-08

Right now, in Firefox Nightly they implemented a fix for the fallback short-circuiting (where it does not check the fallbacks if it does not have to), which led to my technique breaking there. I [opened an issue about this](https://bugzilla.mozilla.org/show_bug.cgi?id=2051901), however it appears that it is likely they will ship it, as they are fine with the potential interop (only my sites).

I am begrudgingly ok with this too, although it is unfortunate. I truly hope that native conditions will land as soon as possible in Firefox, as until they do, my technique is broken, and I did not yet find a good workaround for this case.

For now, I am tweaking it a bit to not completely blow everything up, and adding warnings in examples that rely on it in this blog.
...
<LiveExample class="example2 require-cycles-interop" shouldHideHTML>
```css
.example2 { --WHEN: }
.example2 .minimal {
	--input: var(--bg-value);
...
<LiveExample class="example3 require-cycles-interop" shouldHideHTML>
```css
.example3 {
	--WHEN: ;
	@supports (top: if(():)) {
