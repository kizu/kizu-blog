...
description: 'Today I’ll share my observation that I initially wanted to work more on, but just had to get out sooner rather than later: we can use the `currentColor` inside `color-mix()`! And with them, we could modify an alpha channel of a color only with CSS.'
tags: ['color-mix', 'currentColor', 'CSS', 'Observation']
pubDate: '2023-11-29'
mastodonPostId: ''

current:
	drinking: 'Karkade with raspberries'
	location: 'At Home'
---
...
But that doesn’t work anywhere except for Chrome Canary (I’ve yet to fill a bug). Or this:
